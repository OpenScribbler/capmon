...
func TestCapmonCmd_HasSubcommands(t *testing.T) {
	// capmonCmd is the root command of the standalone binary; sanity-check
	// the expected subcommand set is registered.
	want := map[string]bool{"verify": false, "fetch": false, "run": false}
	...
	for name, found := range want {
		if !found {
			t.Errorf("subcommand %q not registered on capmonCmd", name)
		}
	}
}
